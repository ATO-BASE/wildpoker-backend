...
export class RoomController {
  ...
  static async createRoom(req: Request, res: Response, _next: NextFunction): Promise<void> {
    ...
    // try {
    //   const { tournamentId, roomPassword, maxMembers = 8, minMembers = 2 } = req.body;
    ...
    //   // Check if user is already in a room for this tournament
    //   const existingRoom = await Room.findOne({
    //     where: sequelize.literal(`JSON_CONTAINS(roomMembers, '${userId}')`),
    //     include: [{
    //       model: Tournament,
    ...
    //   if (existingRoom) {
    //     res.status(400).json({ message: 'User already in a room for this tournament' });
    //     return;
    //   }
    ...
    //   // Update creator's room and tournament state
    //   await User.update({
